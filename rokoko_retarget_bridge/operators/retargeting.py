...
class BuildBoneList(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        fixed = mixamo_tools.force_mixamo_bone_map(context.scene)
        if fixed:
            self.report({'INFO'}, f'Build Bone List finished. Fixed {fixed} Mixamo bone mappings.')
...
class FixMixamoBoneMap(bpy.types.Operator):
    bl_idname = "rsl.fix_mixamo_bone_map"
    bl_label = "Fix Mixamo Bone Map"
    bl_description = "Correct common Mixamo arm/leg mapping mistakes after rebuilding the bone list"
    bl_options = {'REGISTER', 'UNDO', 'INTERNAL'}

    def execute(self, context):
        fixed = mixamo_tools.force_mixamo_bone_map(context.scene)
        self.report({'INFO'}, f'Fixed {fixed} Mixamo bone mappings.')
        return {'FINISHED'}


class CheckMixamoTargetAxis(bpy.types.Operator):
    bl_idname = "rsl.check_mixamo_target_axis"
    bl_label = "Check Target Axis"
    bl_description = "Check whether the target Mixamo rest pose faces -Y with shoulders on X"
    bl_options = {'REGISTER', 'UNDO', 'INTERNAL'}

    def execute(self, context):
        target = get_target_armature()
        if target is None:
            self.report({'ERROR'}, 'No target armature selected.')
            return {'CANCELLED'}
        result = mixamo_tools.analyze_target_axis(target)
        context.scene.rro_bridge.last_status = result["status"]
        self.report({'INFO' if result["ok"] else 'WARNING'}, result["status"])
        return {'FINISHED'}


class FixMixamoTargetAxis(bpy.types.Operator):
    bl_idname = "rsl.fix_mixamo_target_axis"
    bl_label = "Fix Target Axis"
    bl_description = "Rotate the target rig root to match standard Mixamo -Y facing when a known mismatch is detected"
    bl_options = {'REGISTER', 'UNDO', 'INTERNAL'}
    ...
    def execute(self, context):
        ...
        if target is None:
            self.report({'ERROR'}, 'No target armature selected.')
            return {'CANCELLED'}
        result = mixamo_tools.fix_target_axis(target)
        context.scene.rro_bridge.last_status = result["status"]
        if result.get("fixed"):
            self.report({'INFO'}, 'Target axis fixed. ' + result["status"])
        elif result["ok"]:
            self.report({'INFO'}, result["status"])
        else:
            self.report({'WARNING'}, 'No automatic fix applied. ' + result["status"])
        return {'FINISHED'}
...
class RetargetAnimation(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        mixamo_tools.force_mixamo_bone_map(context.scene)
